...
class RegistrationController extends Controller
{
    ...
    public function actionCreate()
    {
       ...
       if ($model->load(Yii::$app->request->post()) ) {
            ...
            $model->father_name = strtoupper($model->father_name);
            $model->mother_name = strtoupper($model->mother_name);
    ...
    {
        $fee = AdmissionFee::find()->where(['batch'=>$id])->andwhere(['record_status'=>'1'])->one();
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            if(!$model->save()){
                print_r($model->errors);die;
                Yii::$app->session->setFlash('danger', 'Failed to Update Registration Details !');
                return $this->redirect(Yii::$app->request->referrer);
            }else{
                Yii::$app->session->setFlash('success', 'Registration  Successfully updated !');
                return $this->redirect(['index']);
            }
        }
